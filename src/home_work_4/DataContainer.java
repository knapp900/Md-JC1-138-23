...
public class DataContainer<T> implements Iterable<T> {
    ...
    /**
     * Метод сортировки для контейнеров с объектами и передаваемым компаратором.
     *
    ...
    public Iterator<T> iterator() {
        return new Iterator<T>() {
            ...
            // Сохраняет текущий индекс в контейнере.

            @Override
            public boolean hasNext() {

                return current < modCounter;
                // Возвращает true, если в контейнере есть ещё элементы, false если нет.
            }

            @Override
            public T next() {

                if (current >= modCounter) {
                    throw new NoSuchElementException();
                }
                // Выбрасывает исключение NoSuchElementException, если в контейнере больше нет элементов.

                current++;
                return data[current - 1];
                // Возвращает следующий элемент в контейнере и увеличивает текущий индекс.
            }
        };
    }
